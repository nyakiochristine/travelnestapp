...
function SmartPlanner({ token, onCreated }) {
  ...
  return <div className="tool-card planner-v2">
    ...
    {result && <section className="planner-result"><div><span className="badge">YOUR PLAN</span><h4>{result.itinerary.title}</h4><p>{result.summary.days} days · {result.summary.pace} pace · {result.summary.interestMatch}</p><small className="planner-distance-note">⌖ {result.summary.nearbyStops} nearby match{result.summary.nearbyStops === 1 ? '' : 'es'} found within {result.summary.distanceLimitKm} km.</small>{result.summary.usedInterestFallback && <small className="planner-fallback-note">No nearby places matched your selected interest, so we included other local highlights instead of adding a far-away stop.</small>}</div>{result.dailyPlan.map(day => <article key={day.day}><strong>Day {day.day} — {day.theme}</strong><p>{day.stops.join(' → ')}</p><small>{day.rationale}</small></article>)}</section>}
  </div>;
}
